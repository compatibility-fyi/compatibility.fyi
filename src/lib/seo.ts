...
export function getProjectSeoMetadata(
  ...
  const generatedDescription = `${project.name} version compatibility with ${formatList(dependencyNames)}. Source-backed constraints for ${versions} ${versions === 1 ? 'version group' : 'version groups'}.`;
...
export function getDependencySeoMetadata(
  ...
  const fallback = `${entries.length} ${project.name} version groups include documented ${dependencyName} compatibility constraints, notes, sources, and verification dates.`;
